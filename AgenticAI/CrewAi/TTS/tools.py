...
class VoiceConfig(BaseModel):
    ...
    stability: float = 0.45  # sen
    similarity_boost: float = 0.85  # seen
    style: float = 0.65  # seen
    use_speaker_boost: bool = True  # seen
    model_id: str = "eleven_multilingual_v2" # seen
    output_format: str = "mp3_44100_128" # seen 
    apply_text_normalization: str = "auto"  # seen
...
class PodcastAudioGenerator(BaseTool):
    ...
    model_config = ConfigDict(arbitrary_types_allowed=True)  # arbitrary_types_allowed=True  means to accespt non standard data types 
    # new syntaxe of pydantic equivalent to  : 
    # class MyModel(BaseModel):
    # class Config:
    #     arbitrary_types_allowed = True
    
    api_key: str = Field(default_factory=lambda: os.getenv("ELEVEN_LABS_KEY"))
